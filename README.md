...
![JC4880P443C — ESP32-P4 + MIPI-DSI дисплей в работе](images/madESP32P4_1.jpg)
...
- **ESP-IDF 5.5.4** — проект использует API `i2c_master` из IDF 5.x
...
1. Создайте новый проект **eez+lvgl** в EEZ Studio (при создании выберите `lvgl ^9.5.0`)
2. Соберите проект в EEZ Studio (Build → Build)
3. Скопируйте папку `src/` из проекта EEZ Studio в `main/` этого репозитория, заменив существующую:

```bash
# Из папки проекта EEZ Studio:
cp -r ./ваш_проект_eez/src/ ./main/src/
```

4. Пересоберите проект ESP-IDF:

```bash
idf.py build
```

В процессе сборки EEZ Studio автоматически генерирует массивы изображений (`ui_image_*.c`), стили, экраны и весь остальной необходимый UI-код. Порт EEZ UI (`eez_ui_port.c`, `eez_ui_runtime.c`) обеспечивает совместимость между сгенерированным EEZ кодом и средой выполнения ESP-IDF. Файлы `main/src/ui/` содержат:
...
> **Важно:** Версия LVGL в проекте EEZ Studio должна совпадать с версией в проекте (`^9.5.0`). Несовместимости между LVGL 8.x и 9.x сломают сборку.
